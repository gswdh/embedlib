...
#ifdef DEBUG
            log_info(
                LOG_TAG, "CHRG_PollForOTGMode: Could not read registers via I2C. %u.\n", status);
#endif
            return status;
        }
...
#ifdef DEBUG
            log_info(LOG_TAG,
                     "CHRG_PollForOTGMode: Poll timed out, OTG error. State = %u\n",
                     (data & 0x07));
#endif
...
#ifdef DEBUG
        log_info(
            LOG_TAG, "CHRG_VerifyCHGMode: Could not read registers. Error code = %u.\n", status);
#endif
        return status;
    }
...
#ifdef DEBUG
        log_info(LOG_TAG,
                 "CHRG_VerifyCHGMode: Battery being conditioned, low VBAT or short detected.\n");
#endif
        break;
...
#ifdef DEBUG
            log_info(LOG_TAG,
                     "CHRG_VerifyOTGMode: ACT in OTG_HICCUP mode. Output power limit reached.\n");
#endif
        }
...
#ifdef DEBUG
            log_info(LOG_TAG,
                     "CHRG_VerifyOTGMode: Output voltage below 92%% of OTG voltage setting.\n");
#endif
        }
...
#ifdef DEBUG
    log_info(LOG_TAG,
             "CHRG_EnableCharging: Setting the max charge current to %fA.\n",
             max_charge_current);
#endif
...
#ifdef DEBUG
    log_info(
        LOG_TAG, "CHRG_EnableCharging: Setting the max input current to %fA.\n", max_input_current);
#endif
...
#ifdef DEBUG
        log_info(LOG_TAG,
                 "CHRG_EnableCharging: Charge mode commmand unsuccessful. Error code = %u.\n",
                 status);
#endif
        return status;
    }
...
#ifdef DEBUG
        log_info(LOG_TAG,
                 "CHRG_EnableCharging: Entered charge mode unsuccessfully. Error code = %u.\n",
                 status);
#endif
...
#ifdef DEBUG
        log_info(LOG_TAG,
                 "CHRG_EnableCharging: status bytes 0x02 = %u, 0x03 = %u, 0x04 = %u, 0x05 = %u, "
                 "0x06 = %u.\n",
                 data[0],
                 data[1],
                 data[2],
...
#ifdef DEBUG
        log_info(LOG_TAG,
                 "CHRG_EnterOTG: Error, set output voltage for OTG should be be between 4.5 and "
                 "15.5V, not %f.\n",
                 voltage);
#endif
...
#ifdef DEBUG
        log_info(LOG_TAG,
                 "CHRG_EnterOTG: Error, requested output current out of range, should be below "
                 "3.01A, not %f.\n",
                 max_current);
#endif
        return ACT_OTG_VOLTAGE_OOR;
...
#ifdef DEBUG
        log_info(
            LOG_TAG, "CHRG_EnterOTG: Entered OTG mode unsuccessfully. Error code = %u.\n", status);
#endif
        return status;
    }
...
#ifdef DEBUG
        log_info(LOG_TAG,
                 "CHRG_EnableIdle: Entered idle mode unsuccessfully. Error code = %u.\n",
                 status);
#endif
        return status;
    }
...
#ifdef DEBUG
        log_info(
            LOG_TAG, "CHRG_EnterHiZ: Entered HiZ mode unsuccessfully. Error code = %u.\n", status);
#endif
        return status;
    }
...
act_error_t CHRG_GetStatus() { return ACT_OK; }
...
/*
   act_error_t CHRG_GetVBAT(float *vbat)
   {
                ...
                {
 #ifdef DEBUG
                                log_info(LOG_TAG, "CHRG_GetVBAT: Could not read registers via I2C.
...
act_error_t CHRG_GetADCResults(CHRG_ADCResults *res)
{
    ...
    res->t_th_celcius = (12.3 * res->t_th_celcius * res->t_th_celcius) + (-63 * res->t_th_celcius) +
                        (70.5); // From volts to celcius
