...
void __attribute__((weak)) log_transmit(const char *log)
{
	return;
}
...
void log_message(int level, const char *tag, const char *fmt, ...)
{
	...
	char log_buffer[128];
	int offset = 0;
	offset += snprintf(log_buffer + offset, sizeof(log_buffer) - offset, "%s%s [%010lu]   %s ",
					   log_level_colors[level], log_level_strings[level], uptime, tag);
