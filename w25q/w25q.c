...
w25q_error_t w25q_init(w25q_handle_t *handle)
{
    ...
    if (handle->initialised == true)
    {
        return W25Q_OK;
    }

    w25q_error_t error = w25q_spi_init();
    if (error != W25Q_OK)
    {
        return error;
    }
...
{
    if ((handle == NULL) || (handle->initialised == false) || (manufacturer_id == NULL) ||
        (device_id == NULL))
    {
        return W25Q_ERROR_INVALID_PARAM;
    }
    ...
    w25q_error_t result      = w25q_spi_tx_rx((const uint8_t *)&cmd, 1u, response, 4U);
    if (result != W25Q_OK)
    {
        return result;
    }
...
{
    if (!handle || !handle->initialised || !status)
    {
        return W25Q_ERROR_INVALID_PARAM;
    }
...
{
    if (!handle || !handle->initialised || !data || size == 0)
    {
        return W25Q_ERROR_INVALID_PARAM;
    }
