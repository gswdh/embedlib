...
static bms_error_t bms_full_reset()
{
...
#ifdef DEBUG
				log_fatal(LOG_TAG, "Cannot recover from %u fault in PROTALRT reg.\n", bms_revive_blacklist[i]);
#endif
...
#ifdef DEBUG
		log_info(LOG_TAG, "Found no issues that cannot be repaired. Fully resetting the BMS to repair.\n");
#endif
...
bms_error_t bms_get_voltage(float *voltage)
{
	...
	{
#ifdef DEBUG
		log_error(LOG_TAG, "Error reading raw voltage data from the BMS, error code = %u.\n", status);
#endif
		return status;
	}
...
bms_error_t bms_get_full_capacity(float *full_cap)
{
	...
	{
#ifdef DEBUG
		log_error(LOG_TAG, "Error reading full capacity data from the BMS, error code = %u.\n", status);
#endif
		return status;
	}
